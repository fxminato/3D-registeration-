...
def ProxJ(V0, gamma):
    ...
    m3 = V0[2, :, :, :, :]
    gamma=gamma/2
    a[3, :, :, :, :] = cubic(4 * gamma - f0, 4 * gamma * gamma - 4 * gamma * f0,\
...
def dct4(xx):
    '''
    Compute 4-dimensional discrete cosine transform
    Input: xx(size P*Q*R*T)
    Output: x(size P*Q*R*T)
    '''
    x = xx.copy()
...
def idct4(xx):
    '''
    Compute 4-dimensional inverse discrete cosine transform
    Input: xx(size P*Q*R*T)
    Output: x(size P*Q*R*T)
    '''
    x = xx.copy()
...
if __name__ == "__main__":
    ...
    '''
    with mrcfile.open('/home/fanxiao/data/emd_21546.map', permissive=True, mode='r') as mrc:
    ...
    perform primal-dual algorithm
    '''
    U, V = perform_primal_dual(U0, 100)
    '''
    postprocessing data
    '''
