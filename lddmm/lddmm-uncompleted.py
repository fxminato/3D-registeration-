...
def loadimg(path):
    ...
    img_grey = img[:, :, 0]
    return img_grey/ 255.
def LDDMM(I0,I1, T = 32, maxiter = 200, lr = 5e-3, sigma = 1.0, alpha = 1.0, gamma = 1.0, eps = 1e-3):
    r'''
...
if __name__ == '__main__':
    ...
    c[15:20,29:35]=0.5
    d[15:20,33:39]=0.5
    v, Phi0, Phi1, J0, J1=LDDMM(a,b, T = 32, maxiter = 100, sigma =0.1, alpha =20,lr=0.05)
    #J=J0[:,0,:,:]
    J=np.zeros([96,60,60])
    J[0:32,:,:]=J0[:,0,:,:]
    v, Phi0, Phi1, J0, J1 = LDDMM(b, c, T=32, maxiter=100, sigma=0.1, alpha=20, lr=0.05)
    J[32:64, :, :] = J0[:, 0, :, :]
    v, Phi0, Phi1, J0, J1 = LDDMM(c, d, T=32, maxiter=100, sigma=0.1, alpha=20, lr=0.05)
    J[64:96, :, :] = J0[:, 0, :, :]
